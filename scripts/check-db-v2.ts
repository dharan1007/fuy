...
const LOG_FILE = 'scripts/db-results-v2.txt';
...
async function main() {
    fs.writeFileSync(LOG_FILE, 'Starting DB Check V2...\n');
    log('Checking for bad URLs in database (Media, Chan, Profile)...');
    ...
    try {
        ...
        if (badMedia.length > 0) {
            log('Found bad Media: ' + JSON.stringify(badMedia, null, 2));
        } else {
