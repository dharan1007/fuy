...
class AIActionHandlers {
  private navigationService: INavigationService | null = null;

  setNavigationService(navigationService: INavigationService) {
  ...
  private async handleCreateRoutine(
    ...
    try {
      ...
      // Navigate to Dashboard (home/routine screen)
      if (this.navigationService) {
        this.navigationService.navigateToTab('Dashboard');
      }
  ...
  private async handleSearchProducts(payload: SearchAction): Promise<any> {
    try {
      ...
      // For now, we navigate to the shop screen with search query
      console.log('Searching products:', query);

      if (this.navigationService) {
        this.navigationService.navigateToTab('Social');
      }
  ...
  private async handleSearchPosts(payload: SearchAction): Promise<any> {
    try {
      ...
      // Use the searchMoments endpoint from apiService
  ...
  private async handleFeatureGuide(payload: GuideAction): Promise<any> {
    try {
      ...
      const guides: Record<string, Record<string, any>> = {
        ...
        essenz: {
          beginner: {
            title: 'Essenz Challenges',
            steps: [
              'Navigate to Essenz tab',
              'Browse available challenges',
              'Pick a challenge that interests you',
              'Complete the daily steps',
              'Earn rewards and build streaks',
            ],
            tips: [
              'Start with easier challenges',
              'Build momentum with consistent daily effort',
              'Challenge friends for extra rewards',
            ],
          },
        },
        social: {
      ...
      if (this.navigationService) {
        this.navigationService.navigate('GuideDetail', {
          feature,
          guide,
          level,
        });
